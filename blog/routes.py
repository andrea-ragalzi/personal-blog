...
def homepage():
    ...
    posts = Post.query.order_by(Post.created_at.desc()).paginate(page=page_number, per_page=6 , error_out=True)
    if posts.has_next:
    ...
    return render_template(
        'homepage.html', posts=posts, current_page=page_number, next_page=next_page, previous_page=previous_page)
    ...
    if form.validate_on_submit():
        ...
        if form.image.data:
            try:
                ...
                db.session.commit()
                flash(
                    'There was a problem with the image upload. Please change the image and try again')
